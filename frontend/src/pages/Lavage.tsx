...
export default function Lavage() {
  ...
  useEffect(() => {
    fetch(apiUrl('/api/services'))
      .then(res => res.json())
      .then(data => setServices(data.filter((s: any) => s.category === 'Lavage')))
      .catch(console.error);
  }, []);
    ...
    <div className="w-full bg-[#050505]">
      ...
      <div className="relative w-full h-[70vh] min-h-[500px] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="https://images.unsplash.com/photo-1520340356584-f9917d1eea6f?auto=format&fit=crop&q=80" alt="Lavage Auto" className="w-full h-full object-cover object-center opacity-30" />
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <span className="text-[#16A34A] font-bold tracking-widest text-sm uppercase mb-4 block">MAGIC AUTO LAVAGE</span>
            <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 leading-tight tracking-tight">
              Votre voiture mérite <br/>
              <span className="text-white">un lavage impeccable.</span>
            </h1>
            <p className="text-xl text-gray-300 mb-10 max-w-2xl leading-relaxed">
              Des prestations de lavage pensées pour redonner à votre véhicule tout son éclat. Une attention minutieuse aux détails pour un résultat parfait.
            </p>
            {services.length > 0 && (
              <div className="flex flex-col sm:flex-row gap-4">
                <Link to={`/rendez-vous?service=${services[0]?.id}`} className="px-8 py-4 bg-[#16A34A] text-[#050505] font-bold rounded-md hover:bg-[#158f41] transition flex items-center justify-center text-lg">
                  Réserver mon lavage <ArrowRight className="ml-2 w-5 h-5" />
                </Link>
              </div>
            )}
          </div>
      ...
      <div className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-gray-400 text-lg">Choisissez la formule de lavage adaptée à vos besoins. Nos équipes utilisent des produits haut de gamme pour protéger votre carrosserie.</p>
        </div>
          ...
          {services.map((s: any) => (
            <div key={s.id} className="group bg-[#0B0B0B] border border-white/5 rounded-2xl overflow-hidden flex flex-col hover:border-[#16A34A]/50 transition-all duration-300 hover:-translate-y-2 shadow-lg">
              <div className="h-48 overflow-hidden relative">
                 {/* Fallback image if no specific service image exists */}
                 <img src={s.image || "https://images.unsplash.com/photo-1601362840469-51e4d8d58785?auto=format&fit=crop&q=80"} alt={s.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-80" />
